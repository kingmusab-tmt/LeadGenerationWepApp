  ...
  try {
    ...
    if (callStatus === "no-answer") {
      ...
      if (forwardingType === "direct") {
        ...
        // Determine the next buyer in the round-robin queue
        const lastAssignedIndex = seller.lastAssignedIndex || 0; // Default to 0 if undefined
        const nextBuyerIndex = (lastAssignedIndex + 1) % buyers.length;
        const nextBuyer = buyers[nextBuyerIndex];

        // Update seller's last assigned index
        seller.lastAssignedIndex = nextBuyerIndex;
        await seller.save();
        ...
        const newCall = new Call({
          callSid: `${callSid}-${nextBuyerIndex}`, // New CallSid for this forwarded leg
          userId: seller._id,
